...
Require $bandit 'PeasantRebelBanditStrongholdService.TryCreate(' `
    'Bandit entry must use the stronghold creation transaction.'
...
Require $rulerProjection 'ComposeCeremonialTitle(pKingdom, false)' `
    'The shared ruler read model must project the stronghold ruler title.'
...
Require $heirProjection 'ComposeCeremonialTitle(pKingdom, true)' `
    'The shared heir read model must project the stronghold heir title.'
...
Require $stronghold 'RecordSuppressionChronicles(' `
    'Bandit suppression history must use the unified stronghold fall path.'
