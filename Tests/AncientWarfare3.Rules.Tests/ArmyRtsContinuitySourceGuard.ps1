...
Require-Contains $controller 'TryIssueVanillaCityAttackOrder(army, record.Mission,' 'Captain recovery must republish the locked target to vanilla movement.'
Require-Contains $controller 'ReleaseActor(captain);' 'Captain recovery must release any stale custom RTS job.'
Require-Contains $controller 'RequestRouteReplan(pArmyId,' 'Route recovery must request bounded same-target replanning.'
