...
internal static class AW_VanillaLookupOptimizationPatch
{
    ...
    {
        if (__instance == null || pActor?.current_tile?.chunk == null) return true;
    ...
    {
        if (pChunk?.tiles == null || pChunk.tiles.Length == 0 || string.IsNullOrEmpty(pType)) return true;
    ...
    {
        if (pActor?.current_tile?.chunk == null) return true;
