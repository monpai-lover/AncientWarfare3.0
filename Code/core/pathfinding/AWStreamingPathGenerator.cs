...
namespace AncientWarfare3.core.pathfinding
{
    public sealed class AWStreamingPathGenerator : IAWPathSegmentGenerator
    {
            ...
            CancellationToken pCancellation, int pMaximumSteps)
        {
            ...
            try
            {
...
#else
                currentTerrainRevision = pRequest.WorldKind ==
                    AWPathWorldKind.SubWorld
                    ? pRequest.TerrainRevision
...
#endif
                // A traversal generation is scoped to the world that built
                // it. Keep legacy rule-test generations (0) compatible, but
                // reject a real snapshot from a previous world before cache
                // or path search can consume it.
                bool isSubWorld = pRequest.WorldKind == AWPathWorldKind.SubWorld;
                if ((!isSubWorld && pRequest.Generation.WorldGeneration != 0L &&
                     pRequest.Generation.WorldGeneration != currentWorldGeneration) ||
                    (!isSubWorld && pRequest.ReuseKey.WorldGeneration != 0L &&
                     pRequest.ReuseKey.WorldGeneration != currentWorldGeneration) ||
                    (pRequest.Generation.WorldGeneration != 0L &&
                     pRequest.ReuseKey.WorldGeneration != 0L &&
                     pRequest.Generation.WorldGeneration !=
                         pRequest.ReuseKey.WorldGeneration))
                    return AWPathGenerationResult.Failure(
                        ...
                        out bool cachedComplete))
                {
                    ...
                    return BuildSuccess(pRequest, cachedEnd, cachedComplete,
                        cachedSteps, 0, AWPathGenerationKind.Search);
                }
                ...
                if (pRequest.StartTileId == pRequest.TargetTileId)
                    return BuildSuccess(pRequest, pRequest.TargetTileId, true,
                        Array.Empty<AWPathStep>(), 0,
                        ...
                        pRequest.Profile.IsMilitary, pRequest.Options.BoundedMilitaryWater))
                {
                    ...
                    AWTransportRouteSnapshot route = pRequest.TransportRoute;
                    AWPathStep transportStep = route.IsValid
                        ? new AWPathStep(target.Id, AWMovementMethod.Transport,
                            transportEstimate, -1L, AWPathTileFlags.None,
                            route.EntryPortalId, route.ExitPortalId,
                            route.EntryLandTileId, route.PickupSeaTileId,
                            route.DestinationSeaTileId,
                            route.LandingLandTileId)
                        : new AWPathStep(target.Id, AWMovementMethod.Transport,
                            transportEstimate);
                    return BuildSuccess(pRequest, target.Id, true,
                        new[] { transportStep }, 0,
                        AWPathGenerationKind.Transport);
                }
                        ...
                        out int straightEnd))
                {
                    ...
                    return BuildSuccess(pRequest, straightEnd, straightReachedTarget,
                        straightSteps, 0, AWPathGenerationKind.StraightLine);
                }
                ...
                return BuildSuccess(pRequest, endTileId, reachedTarget, steps,
                    result.ExpandedNodes, corridor != null
        ...
        private static AWPathGenerationResult BuildSuccess(
            AWPathRequest pRequest, int pEndTileId, bool pReachedTarget,
            IReadOnlyList<AWPathStep> pSteps, int pExpandedNodes,
            AWPathGenerationKind pKind)
        {
            AWTraversalState state = new AWTraversalState(
                pRequest?.Profile.Stamina ?? 0f,
                pRequest?.Profile.Health ?? 0f, 0f);
            for (int index = 0; index < (pSteps?.Count ?? 0); index++)
                state = AWTraversalRules.AdvanceState(state,
                    pRequest.Profile, pSteps[index].Estimate);
            return AWPathGenerationResult.Success(pEndTileId,
                pReachedTarget, pSteps, pExpandedNodes, pKind,
