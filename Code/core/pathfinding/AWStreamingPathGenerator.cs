    ...
    public sealed class AWStreamingPathGenerator : IAWPathSegmentGenerator
    {
            ...
            CancellationToken pCancellation, int pMaximumSteps)
        {
            ...
            try
            {
                ...
                // The worker normally asks for bounded segments. Keep the
                // legacy whole-path entry point on the full A* contract and
                // only shortcut ordinary ambient land walkers.
                if (pMaximumSteps != int.MaxValue &&
                    AWPathOptimizationRules.ShouldUseStraightSegment(
                        pRequest.WorkClass,
                        ...
                        pRequest.Profile.CanFly) &&
                    TryBuildStraightSegment(pRequest, start, target,
                        Math.Min(Math.Max(1, pMaximumSteps), 16), _config,
                        out AWPathStep[] straightSteps, out int straightEnd))
                {
#if !AW3_RULES_TESTS
                    AWPathfindingBootstrap.PathDiagnostics.OnStraightSegment();
#endif
                    return AWPathGenerationResult.Success(straightEnd,
                        straightEnd == target.Id, straightSteps);
                }
            ...
            out AWPathStep[] pSteps, out int pEndTileId)
        {
            ...
            while ((x != targetX || y != targetY) && count < pMaximumSteps)
            {
                ...
                if (!TryGetAt(pRequest.Generation, x, y,
                        out AWTileTraversalSnapshot next) ||
                    !IsFastTileSafe(next, pRequest.Profile,
                        pRequest.Options))
                {
                    pSteps = null;
                    pEndTileId = pStart.Id;
                    return false;
                }

                bool diagonal = previousX != x && previousY != y;
                if (diagonal &&
                    (!TryGetAt(pRequest.Generation, x, previousY,
                         out AWTileTraversalSnapshot sideX) ||
                     !TryGetAt(pRequest.Generation, previousX, y,
                         out AWTileTraversalSnapshot sideY) ||
                     !IsFastTileSafe(sideX, pRequest.Profile,
                         pRequest.Options) ||
                     !IsFastTileSafe(sideY, pRequest.Profile,
                         pRequest.Options)))
                {
                    pSteps = null;
                    pEndTileId = pStart.Id;
                ...
                pRequest.Generation.TryGet(currentId,
                    out AWTileTraversalSnapshot current);
                AWTraversalEstimate estimate = AWTraversalRules.Estimate(
                    current, next, pRequest.Profile, pRequest.Options,
                    pConfig);
                buffer[count++] = new AWPathStep(next.Id,
                    AWMovementMethod.Walk, estimate,
                    pPlannedTileFlags: AWPathTileFlagsExtensions.FromSnapshot(next));
                currentId = next.Id;
            }
        ...
        private static bool TryGetAt(AWTraversalGeneration pGeneration,
            int pX, int pY, out AWTileTraversalSnapshot pTile)
        {
            pTile = default;
            if (pGeneration == null || pX < 0 || pY < 0 ||
                pX >= pGeneration.Width || pY >= pGeneration.Height)
                return false;
            return pGeneration.TryGet(pY * pGeneration.Width + pX,
                out pTile);
        }
        ...
        {
            if (!AWTraversalRules.CanEnter(pTile, pProfile, pOptions) ||
                !pTile.HasType || pTile.DamageUnits)
                return false;
            if (pTile.Block && !pOptions.WalkOnBlocks && !pProfile.CanFly)
                return false;
            if (pTile.Lava && pProfile.DiesInLava &&
                !pOptions.WalkOnLava)
                return false;
            if (pTile.Fire && !pProfile.ImmuneToFire && !pProfile.Burning)
                return false;
            // This shortcut does not carry A*'s water-run and drowning state.
            // All liquid traversal therefore remains on the full search path.
            if (pTile.Liquid || pTile.Ocean) return false;
            return true;
        }
