...
{
    // Kept in the same ordered, main-thread lifecycle as Cultiway master.
    // AW3 only adds quarantine/repair for actors whose kingdom reference is
    // temporarily invalid during affiliation transfer or save loading.
    internal sealed class AWCooperativeWorldMaintenanceRunner
    {
        ...
        public bool Step()
        {
            switch (_stage)
            {
                ...
                case MaintenanceStage.DestroyMetaObjects:
                    if (!_windowOnScreen)
                    {
                        _world.checkMetaObjectsDestroy();
                    }
        ...
        private void ProcessActorMetaBatch()
        {
            ...
            for (; _index < end; _index++)
            {
                ...
                if (!actor.isAlive() ||
                    ...
                    kingdom.asset == null || kingdom.isRekt())
                {
                    if (actor.isAlive() &&
                        (kingdom == null || kingdom.data == null ||
                         kingdom.asset == null || kingdom.isRekt()))
            ...
            BaseSystemManager pManager)
        {
            ...
            if (!DirtyManagerPhaseNames.TryGetValue(type,
                    out string phase))
            {
